...
export class GitService {
  ...
  async status(id: string): Promise<GitStatus> {
    ...
    for (let index = 0; index < records.length; index += 1) {
      ...
      const conflicted = indexStatus === 'U' || worktreeStatus === 'U' ||
  ...
  async history(id: string, limit = 100): Promise<GitCommit[]> {
    ...
    if (result.code !== 0) return []
    return result.stdout.split('\x1e').filter((item) => item.trim()).map((record) => {
      const [hash, shortHash, parents, author, authorEmail, date, subject, body, refs] = record.trim().split('\x1f')
      return {
        hash: hash ?? '', shortHash: shortHash ?? '', parents: parents?.split(' ').filter(Boolean) ?? [],
        author: author ?? '', authorEmail: authorEmail ?? '', date: date ?? '', subject: subject ?? '',
        body: body?.trim() ?? '', refs: refs?.split(',').map((ref) => ref.trim()).filter(Boolean) ?? [],
      }
    })
  }
  ...
  async branches(id: string): Promise<GitBranch[]> {
    const checkout = await this.resolveCheckout(id)
    if (!checkout) return []
    const format = ['%(refname)', '%(refname:short)', '%(HEAD)', '%(upstream:short)', '%(upstream:track,nobracket)', '%(objectname:short)', '%(subject)'].join('%x1f')
    ...
    return result.stdout.split('\n').filter(Boolean).map((line) => {
      ...
      return { name, current: current === '*', remote: fullName.startsWith('refs/remotes/'),
        ...(upstream ? { upstream } : {}), ahead, behind, lastCommit: `${hash ?? ''} ${subject ?? ''}`.trim() }
    })
  }

  stage(id: string, paths: string[]) { return this.mutate(id, ['add', '--', ...safePaths(paths)]) }
  unstage(id: string, paths: string[]) { return this.mutate(id, ['restore', '--staged', '--', ...safePaths(paths)]) }
  discard(id: string, paths: string[]) { return this.mutate(id, ['restore', '--worktree', '--', ...safePaths(paths)]) }
  stash(id: string, message?: string) { return this.mutate(id, ['stash', 'push', ...(message?.trim() ? ['-m', message.trim().slice(0, 200)] : [])]) }
  stashPop(id: string) { return this.mutate(id, ['stash', 'pop']) }
  ...
  createBranch(request: GitBranchCreateRequest) {
    if (!validRef(request.name) || (request.startPoint && !validRef(request.startPoint))) return Promise.resolve(failure('Invalid branch name.'))
    return this.mutate(request.checkoutId, [request.switchTo ? 'switch' : 'branch', ...(request.switchTo ? ['-c'] : []), request.name,
      ...(request.startPoint ? [request.startPoint] : [])], true)
  }
  switchBranch(id: string, branch: string) { return validRef(branch) ? this.mutate(id, ['switch', branch], true) : Promise.resolve(failure('Invalid branch name.')) }
  deleteBranch(id: string, branch: string) { return validRef(branch) ? this.mutate(id, ['branch', '-d', branch], true) : Promise.resolve(failure('Invalid branch name.')) }
  renameBranch(id: string, oldName: string, newName: string) { return validRef(oldName) && validRef(newName) ? this.mutate(id, ['branch', '-m', oldName, newName], true) : Promise.resolve(failure('Invalid branch name.')) }
  mergeBranch(id: string, branch: string) { return validRef(branch) ? this.mutate(id, ['merge', '--no-edit', branch], true) : Promise.resolve(failure('Invalid branch name.')) }
  fetch(id: string) { return this.mutate(id, ['fetch', '--prune']) }
  pull(id: string) { return this.mutate(id, ['pull', '--ff-only'], true) }
  push(id: string) { return this.mutate(id, ['push']) }
  ...
  async createWorktree(request: GitWorktreeCreateRequest): Promise<GitOperationResult> {
    ...
    if (!main || !validRef(request.branch) || (request.startPoint && !validRef(request.startPoint))) return failure('Invalid worktree request.')
    const target = path.resolve(request.path)
    const args = ['worktree', 'add', ...(request.createBranch ? ['-b', request.branch] : []), target,
  ...
  async clone(request: GitCloneRequest): Promise<ProjectConfig | null> {
    ...
    return { id: `project-${Date.now().toString(36)}`, name: request.name?.trim().slice(0, 120) || path.basename(directory),
      path: directory, framework: 'custom', autoStart: false, tasks: [] }
  }
  ...
  private async mutate(id: string, args: string[], blockWhenBusy = false): Promise<GitOperationResult> {
    ...
    if (blockWhenBusy && checkout.busy) return failure('Stop tasks and agents using this checkout before changing its branch.')
    if (this.mutationLocks.has(checkout.projectId)) return failure('Another Git operation is already running for this project.')
...
function validRef(value: string): boolean { return SAFE_REF.test(value) && !value.endsWith('.') && !value.endsWith('/') }
function safePaths(paths: string[]): string[] { return paths.filter((item) => typeof item === 'string' && item.length > 0 && item.length <= 4096 && !item.includes('\0')).slice(0, 500) }
function failure(output: string): GitOperationResult { return { ok: false, output, conflict: false } }
function emptyStatus(): GitStatus { return { isRepository: false, branch: '', ahead: 0, behind: 0, clean: true, detached: false, files: [] } }
function parseStatusHeader(header: string) {
  ...
  return { branch: branchPart.replace(/^No commits yet on /, ''), upstream, detached,
    ahead: Number(tracking.match(/ahead (\d+)/)?.[1] ?? 0), behind: Number(tracking.match(/behind (\d+)/)?.[1] ?? 0) }
}
async function hasConflicts(cwd: string): Promise<boolean> { return (await run('git', ['diff', '--name-only', '--diff-filter=U'], cwd)).stdout.trim().length > 0 }

function run(command: string, args: string[], cwd: string, timeoutMs = 120_000): Promise<CommandResult> {
  return new Promise((resolve) => {
    const child = spawn(command, args, { cwd, windowsHide: true, env: { ...process.env, GIT_TERMINAL_PROMPT: '0' }, stdio: ['ignore', 'pipe', 'pipe'] })
    let stdout = ''
    let stderr = ''
